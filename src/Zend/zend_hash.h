...
#define HASH_FLAG_APPLY_PROTECTION (1<<1) //是否开启递归保护
#define HASH_FLAG_PACKED           (1<<2) //是否是packed array
#define HASH_FLAG_INITIALIZED      (1<<3) //是否已初始化
#define HASH_FLAG_STATIC_KEYS      (1<<4) /* long and interned strings */ //标记HashTable的key是否为long key或者内部字符串k
